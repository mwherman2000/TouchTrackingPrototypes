...
namespace BTTN4KNFE
{
	...
	public class BTTN4KNFEFactory
	{
		...
		public static void SaveNfe(string filename, string nfeJson)
		{
			...
			Debug.WriteLine(Android.OS.Environment.ExternalStorageDirectory.AbsolutePath);
			Debug.WriteLine(Android.OS.Environment.DirectoryDownloads);
			...
			Debug.WriteLine(nfePath);

			File.WriteAllText(nfePath, nfeJson);
		}

		public static void PostNfe(string url, string nfeJson)
        {
			using (var httpClient = new HttpClient())
			{
				string agentUrl = String.Format("{0}/SendNFEEnvelopeToStorage/", url);
